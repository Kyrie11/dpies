...
python "${NUPLAN_DEVKIT_ROOT}/nuplan/planning/script/run_simulation.py" \
  +simulation="${SIMULATION_CONFIG:-closed_loop_reactive_agents}" \
  ...
  experiment_name="${EXPERIMENT_NAME:-dpies_closed_loop_reactive_idm}" \
  group="${GROUP:-dpies}" \
  output_dir="${OUTPUT_DIR:-${NUPLAN_EXP_ROOT}/dpies_closed_loop_reactive_idm}" \
  ${EXTRA_NUPLAN_ARGS:-}
